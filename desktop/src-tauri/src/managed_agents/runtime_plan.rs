...
impl RuntimeExecutionPlan {
    ...
    pub(crate) fn generated_environment_entries(
        &self,
...
fn append_provider_runtime_closure(
    ...
    if prefix.starts_with(b"#!") {
        return append_node_runtime_closure(
            provider,
            source,
            components,
            packages,
...
fn append_node_runtime_closure(
    ...
    let shebang = first_line
        .strip_prefix("#!")
        .ok_or_else(|| "Codex adapter is not a shebang launcher; refusing an incomplete plan".to_string())?
        .trim();
...
fn collect_package_files(
    ...
    let entries = std::fs::read_dir(directory)
        .map_err(|error| format!("failed to read runtime package {}: {error}", directory.display()))?;
...
fn planned_path(node_path: &Path) -> Result<String, String> {
    let node_dir = node_path
        .parent()
        .ok_or_else(|| format!("Node runtime has no parent directory: {}", node_path.display()))?;
...
fn push_unique_component(
    ...
    if !components.iter().any(|existing| existing.path == component.path) {
